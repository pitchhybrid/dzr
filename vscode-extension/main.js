...
const hhmmss = (s) =>
	new Date(s * 1000).toISOString().slice(11, 19).replace(/^00:/, "");
const wait = (ms = 1000) => new Promise((resolve) => setTimeout(resolve, ms));
// still no fetch() in 2023 ?
const fetch = (url, opt, data) =>
...
async function browse(url_or_event_or_ids, label) {
	try {
		if (Array.isArray(url_or_event_or_ids))
			return url_or_event_or_ids.map((id) => ({ id }));
		const ignoreFocusOut = true;
		const url =
			typeof url_or_event_or_ids == "string" ? url_or_event_or_ids : "/";
		const id = url.replace(/\d+/g, "0").replace(/[^\w]/g, "_");
		const menus = conf.get("menus");
		const title = (label || "").replace(/\$\(.+?\)/g, "");
		if (url.endsWith("=") || url.endsWith("/0")) {
			// query step
			...
			const pick =
				menus[id].length > 1 ?
				await vscode.window.showQuickPick(menus[id], {
					title: title || url,
					ignoreFocusOut,
				}) :
				menus[id][0];
			...
			const choices = data.map((entry) => ({
				...entry,
				picked,
				label: (type2icon[entry.type] || "") +
					(entry.title_short || entry.name || entry.title),
				description: [
					entry.artist?.name,
					entry.title_version,
					entry.nb_tracks,
			...
			const picks = await vscode.window.showQuickPick(choices, {
				title: title || url,
				canPickMany,
				ignoreFocusOut,
			});
...
const with_url = async (songs) =>
	songs?.length ?
	await vscode.window.withProgress({ title: "Fetching Song Info...", location },
		async (progress) => {
			try {
				// take 7s (with, or without agent)
				const next = (val) => (
					progress.report({ increment: 100 / 4 }), val
				);
				const gw = async (method, sid, api_token = "", opt = {}, data) =>
					JSON.parse(
						await fetch(
							`${base}&method=${method}&api_token=${api_token}`,
							{
								...opt,
								headers: {
									Cookie: `sid=${sid}`,
				...
				const base = next(
					"https://www.deezer.com/ajax/gw-light.php?input=3&api_version=1.0"
				);
				const DZR_PNG = next(await gw("deezer.ping"));
				const USR_NFO = next(
					await gw("deezer.getUserData", DZR_PNG.SESSION)
				);
				const SNG_NFO = next(
					await gw(
						"song.getListData",
						DZR_PNG.SESSION,
						USR_NFO.checkForm, { method: "POST" },
						JSON.stringify({ sng_ids: songs.map((s) => s.id) })
					)
				);
				const URL_NFO = next(
					JSON.parse(
						await fetch(
							"https://media.deezer.com/v1/get_url", { method: "POST" },
							JSON.stringify({
				...
				const errors = URL_NFO.data
					.map((nfo, i) => [nfo.errors, songs[i]])
					.filter(([err]) => err)
					.map(
						([
							[err], sng
						]) => `${sng.title}: ${err.message} (${err.code})`
					)
					.join("\n");
				if (errors)
					setTimeout(() => vscode.window.showWarningMessage(errors), 500); // can't warn while progress ?
				return songs
					.map(({ id }, i) => ({
						id,
						...
						artists: SNG_NFO.data[i].ARTISTS.map((a) => ({
							id: a.ART_ID,
							name: a.ART_NAME,
							md5: a.ART_PICTURE,
						})),
...
class DzrWebView {
	// can't Audio() in VSCode, we need a webview
	...
	state = new Proxy(this.#state, {
		set: (target, key, value) => {
			target[key] = value;
			if (["queue", "looping"].includes(key)) {
				// persist those values across reboot
	...
	renderStatus() {
		...
		const label = this.state.current ?
			`${this.state.current.title} - ${this.state.current.artists
          ?.map((a) => a.name)
          .join()}` :
			"";
		this.statuses[0].command = this.state.playing ? "dzr.pause" : "dzr.play";
		this.statuses[0].text =
			this.state.ready && (this.state.playing ? "$(debug-pause)" : "$(play)");
		this.statuses[1].tooltip = this.state.ready ?
			label :
			"Initiate interaction first";
		this.statuses[1].text = this.state.ready ?
			label.length < 20 ?
			label :
			label.slice(0, 20) + "…" :
			"$(play)";
		this.statuses[2].text =
			this.state.ready && this.state.queue.length ?
			`${index + 1 || "?"}/${this.state.queue.length} $(chevron-right)` :
			null; //debug-step-over
		this.treeView.description =
			(this.state.queue?.length ?
				`${index + 1 || "?"}/${this.state.queue.length}` :
				"") + ` loop:${this.state.looping}`;
		this.treeView.message = this.state.queue?.length ?
			null :
			"Empty Queue. Add tracks to queue using '+'";
	}
	async show(htmlUri, iconPath) {
		...
		this.panel.webview.onDidReceiveMessage((action, ...args) =>
			this[action] ? this[action](...args) : this.badAction(action)
		);
		this.panel.onDidDispose(() => (this.state.ready = this.panel = null));
		this.post("state", this.state, Object.keys(this.state));
	}
	initAckSemaphore() {
		this.postAck = new Promise((then) => (this.waitAckSemaphore = then));
	}
	...
	player_playing() {
		this.state.ready = this.state.playing = true;
	}
	player_pause() {
		this.state.playing = false;
	}
	player_ended() {
		vscode.commands.executeCommand("dzr.load", null);
	}
	user_interact() {
		this.state.ready = true;
	}
	user_next() {
		vscode.commands.executeCommand("dzr.load");
	}
	error(msg) {
		vscode.window.showErrorMessage(msg);
	}
	badAction(action) {
		console.error(`unHandled action "${action}" from webview`);
	}
	// tree
	...
	treeView = vscode.window.createTreeView("dzr.queue", {
	...
	getTreeItem = (item) => ({
		...
		command: {
			...
			command: "dzr.load",
			tooltip: "Play",
			arguments: [this.state.queue.indexOf(item)],
		},
	});
...
exports.activate = async function(
	...
	conf.get("cbc") ||
		vscode.window.withProgress({ title: "Extracting CBC key...", location },
			async () => {
				...
				if (!js_url)
					return await vscode.window.showErrorMessage(
						"CBC Extract: No JS WebApp found"
					);
				const keys = (await fetch(js_url))
					.toString("utf-8")
					.match(/%5B0x..%2C.{39}%2C0x..%5D/g);
				const [a, b] = keys.map((part) =>
					part
					.slice(3, -3)
					.split("%2C")
					.map((i) => String.fromCharCode(parseInt(i)))
					.reverse()
				);
				...
				const sha = crypto
					...
					.slice(0, 8);
				if (sha != "3ad58d92")
					return await vscode.window.showErrorMessage("Bad extracted key");
	...
	context.subscriptions.push(
		...
		vscode.commands.registerCommand("dzr.show", () =>
			dzr.show(htmlUri, iconUri)
		),
		vscode.commands.registerCommand("dzr.play", () => dzr.post("play")),
		vscode.commands.registerCommand("dzr.pause", () => dzr.post("pause")),
		vscode.commands.registerCommand("dzr.href", (track) =>
			vscode.env.openExternal(
				vscode.Uri.parse(`https://deezer.com/track/${track.id}`)
			)
		),
		vscode.commands.registerCommand(
			"dzr.loopQueue",
			() => (dzr.state.looping = "queue")
		),
		vscode.commands.registerCommand(
			"dzr.loopTrack",
			() => (dzr.state.looping = "track")
		),
		vscode.commands.registerCommand(
			"dzr.loopOff",
			() => (dzr.state.looping = "off")
		),
		vscode.commands.registerCommand("dzr.add", async (path, label) =>
			with_url(await browse(path, label)).then(
				(tracks) => (dzr.state.queue = [...dzr.state.queue, ...tracks])
			)),
		vscode.commands.registerCommand("dzr.remove", async (item, items) =>
			(items || [item]).map((i) =>
				vscode.commands.executeCommand(
					"dzr.removeAt",
					dzr.state.queue.indexOf(i)
				)
			)),
		vscode.commands.registerCommand(
			"dzr.removeAt",
			async (index) =>
				index >= 0 &&
				(dzr.state.queue = [
					...dzr.state.queue.slice(0, index),
					...dzr.state.queue.slice(index + 1),
				])
		),
		vscode.commands.registerCommand(
			"dzr.clear",
			async () => (dzr.state.queue = [])
		),
		vscode.commands.registerCommand("dzr.shareAll", async () =>
			vscode.commands.executeCommand("dzr.share")),
		vscode.commands.registerCommand("dzr.share", async (track, tracks) => {
			const ids = JSON.stringify(
				track ?
				[(tracks || [track]).map((e) => e.id || track.id)] :
				[dzr.state.queue.map((q) => q.id)]
			);
			vscode.env.clipboard.writeText(
				new vscode.Uri("vscode", context.extension.id, "/add", ids).toString()
			);
		}),
		...
		vscode.commands.registerCommand("dzr.load", async (pos) => {
			//pos=null if player_end / pos=undefine if user click
			pos =
				pos ??
				dzr.state.queue.indexOf(dzr.state.current) +
				(dzr.state.looping == "track" ? 0 : 1);
			if (!dzr.state.queue[pos]) {
				// out of bound track
			...
			const md5 = hex(
				crypto.createHash("md5").update(`${dzr.state.current.id}`).digest("hex")
			);
			const key = Buffer.from(
				hex(conf.get("cbc")).map((c, i) => c ^ md5[i] ^ md5[i + 16])
			);
